package com.sjet.auracascade.data;
...
public final class PosUtil {
    ...
    public static Direction directionTo(BlockPos from, BlockPos to) {
        ...
        if (count == 1) {
            for (Direction direction : Direction.values()) {
                if (direction.getXOffset() == (int) Math.signum(xDiff) && direction.getYOffset() == (int) Math.signum(yDiff) && direction.getZOffset() == (int) Math.signum(zDiff)) {
                    return direction;
                }
            }
        }
    ...
    public static Iterable<BlockPos> inRange(BlockPos from, int range) {
        return (Iterable<BlockPos>) BlockPos.getAllInBox(from.add(-range, -range, -range), from.add(range, range, range));
    }
}